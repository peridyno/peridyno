...
void pybind_modeling(py::module& m)
{
	...
	py::class_<dyno::SkinInfo, dyno::OBase, std::shared_ptr<dyno::SkinInfo>>(m, "SkinInfo")
		.def(py::init<>())
		.def("push_back_data", &dyno::SkinInfo::pushBack_Data)
		.def("clear_skin_info", &dyno::SkinInfo::clearSkinInfo)
		.def("size", &dyno::SkinInfo::size)
		.def("is_empty", &dyno::SkinInfo::isEmpty)

		.def_readwrite("V_jointWeight_0", &dyno::SkinInfo::V_jointWeight_0)
		.def_readwrite("V_jointWeight_1", &dyno::SkinInfo::V_jointWeight_1)
		.def_readwrite("V_jointID_0", &dyno::SkinInfo::V_jointID_0)
		.def_readwrite("V_jointID_1", &dyno::SkinInfo::V_jointID_1)
		.def_readwrite("skin_VerticeRange", &dyno::SkinInfo::skin_VerticeRange)
		.def_readwrite("mesh", &dyno::SkinInfo::mesh)
		.def_readwrite("initialPosition", &dyno::SkinInfo::initialPosition)
		.def_readwrite("initialNormal", &dyno::SkinInfo::initialNormal);

	py::enum_<typename dyno::Canvas::Direction>(CANVAS, "Direction")
		.value("x", dyno::Canvas::Direction::x)
		.value("y", dyno::Canvas::Direction::y)
