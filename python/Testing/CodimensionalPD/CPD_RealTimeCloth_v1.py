...
scn.add_node(object)
object.var_file_name().set_value(dyno.FilePath(dyno.get_asset_path() + "cloth_shell/v1/woman_model.obj"))

volLoader = dyno.VolumeLoader3f()
scn.add_node(volLoader)
volLoader.var_file_name().set_value(dyno.FilePath(dyno.get_asset_path() + "cloth_shell/v1/woman_v1.sdf"))
...
# interaction.var_cache_event().set_value(False)
# cloth.state_triangle_set().connect(interaction.in_initial_triangle_set())
# cloth.state_position().connect(interaction.in_position())
# cloth.state_velocity().connect(interaction.in_velocity())
# cloth.state_attribute().connect(interaction.in_attribute())
# cloth.state_time_step().connect(interaction.in_time_step())
# cloth.animation_pipeline().push_module(interaction)
