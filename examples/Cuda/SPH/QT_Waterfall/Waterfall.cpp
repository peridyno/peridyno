...
namespace dyno
{
		...
		: ParticleEmitter<TDataType>()
	{
		...
		auto callback = std::make_shared<FCallBackFunc>(std::bind(&Waterfall<TDataType>::tranformChanged, this));
		...
		hpos.push_back(Coord(0.1f, 0.1f, 0.1f));
		this->mPosition.assign(hpos);
		this->mVelocity.resize(1);
	}
	...
	template<typename TDataType>
	void Waterfall<TDataType>::generateParticles()
	{
