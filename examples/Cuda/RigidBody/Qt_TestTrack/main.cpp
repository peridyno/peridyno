...
std::shared_ptr<SceneGraph> creatCar()
{
	...
	jeep->varLocation()->setValue(Vec3f(0,2,0));

	auto mapper = std::make_shared<DiscreteElementsToTriangleSet<DataType3f>>();
	jeep->stateTopology()->connect(mapper->inDiscreteElements());

	auto elementsRender = std::make_shared<GLSurfaceVisualModule>();
	mapper->outTriangleSet()->connect(elementsRender->inTriangleSet());
	elementsRender->varAlpha()->setValue(0.2);

	jeep->graphicsPipeline()->pushModule(mapper);
	jeep->graphicsPipeline()->pushModule(elementsRender);
	...
	for (uint i = 0; i < N; i++)
	{
		...
		box1.halfLength = Vec3f(1.011, 0.793, 2.4)/1.2;
	...
	auto mesh = scn->addNode(std::make_shared<GltfLoader<DataType3f>>());
	mesh->varFileName()->setValue(getAssetPath() + "gltf/TestTrack/TestTrack.gltf");

	auto toTri = scn->addNode(std::make_shared<TextureMeshToTriangleSetNode<DataType3f>>());
	mesh->stateTextureMesh()->connect(toTri->inTextureMesh());
	toTri->outTriangleSet()->connect(jeep->inTriangleSet());
