...
std::shared_ptr<SceneGraph> createScene()
{
	...
	scn->setGravity(Vec3f(0.0f, -1.5f, 0.0f));
	scn->setLowerBound(Vec3f(-1.0f, 0.0f, -0.1f));
	scn->setUpperBound(Vec3f(1.0f, 2.0f, 0.1f));

	auto semic = scn->addNode(std::make_shared<UpperSemiCircleSampler<DataType3f>>());
	semic->varCenter()->setValue(Vec3f(0.0, 0.9, 0.0));
	semic->varRadius()->setValue(0.9f);
	semic->varDx()->setValue(0.004f);
	semic->varSamplingDistance()->setValue(0.002f);
	semic->varYPlane()->setValue(1.2f);
	semic->setVisible(false);

	auto initialParticles = scn->addNode(std::make_shared<MakeParticleSystem<DataType3f>>());
	semic->statePointSet()->promoteOuput()->connect(initialParticles->inPoints());
	...
	//Create a boundary
	auto boundary = std::make_shared<StaticBoundaryCircleInlet<DataType3f>>();
	...
	//abcWriter->setOutputPath("D:/ACFD/QuicksandInlet/abc");
	...
	//triWriter->setOutputPath("D:/ACFD/QuicksandInlet/obj/");
