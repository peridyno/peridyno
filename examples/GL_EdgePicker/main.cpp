...
int main(int, char**)
{
	...
	auto instanceRender1 = std::make_shared<GLWireframeVisualModule>();
	instanceRender1->setColor(Vec3f(1, 0, 0));
	edgePickerNode->stateSelectedTopology()->connect(instanceRender1->inEdgeSet());
	edgePickerNode->graphicsPipeline()->pushModule(instanceRender1);

	auto instanceRender2 = std::make_shared<GLWireframeVisualModule>();
	instanceRender2->setColor(Vec3f(0, 0, 0));
	edgePickerNode->stateOtherTopology()->connect(instanceRender2->inEdgeSet());
	edgePickerNode->graphicsPipeline()->pushModule(instanceRender2);
