...
T interpolate_value(const Vector<S,3>& point, const CArray3D<T>& grid)
{
   int i,j,k;
   S fi,fj,fk;

   get_barycentric(point[0], i, fi, 0, grid.nx());
   get_barycentric(point[1], j, fj, 0, grid.ny());
   get_barycentric(point[2], k, fk, 0, grid.nz());
...
T interpolate_gradient(Vector<T,3>& gradient, const Vector<S,3>& point, const CArray3D<T>& grid) {
   int i,j,k;
   S fx,fy,fz;
   
   get_barycentric(point[0], i, fx, 0, grid.nx());
   get_barycentric(point[1], j, fy, 0, grid.ny());
   get_barycentric(point[2], k, fz, 0, grid.nz());
