...
namespace dyno
{
    ...
    void EdgeInteraction<TDataType>::calcEdgeIntersectClick() {
        ...
        this->kernel("EdgeRay")->flush(vkDispatchSize(vk_num, 64), points.handle(), intersected.handle(),
        ...
        if (this->varToggleMultiSelect()->getData()) {
            ...
            if (this->edgeIntersectedIndex.size() == 0) {
                ...
                vkFill(*(this->edgeIntersectedIndex.handle()), 0);
            }
            ...
            this->kernel("Merge")->flush(vkDispatchSize(vk_num, 64), this->edgeIntersectedIndex.handle(),
        ...
        this->kernel("EdgeAssignOut")
            ->flush(vkDispatchSize(vk_num, 64), edges.handle(), intersected_edges.handle(),
    ...
    void EdgeInteraction<TDataType>::calcEdgeIntersectDrag() {
        ...
        this->kernel("EdgeBox")
            ->flush(vkDispatchSize(vk_num, 64), points.handle(), edges.handle(), intersected.handle(), unintersected.handle(), &vk_num,
    ...
    void EdgeInteraction<TDataType>::mergeIndex() {
        /*
           ...
           intersected.size(), (int)0, thrust::plus<int>()); DArray<int> outEdgeIndex;
                outEdgeIndex.resize(intersected_size);
                thrust::exclusive_scan(thrust::device, intersected.begin(), intersected.begin() + intersected.size(),
           intersected.begin()); DArray<Edge> intersected_edges; intersected_edges.resize(intersected_size);
           ...
           unintersected.begin(), unintersected.begin() + unintersected.size(), unintersected.begin()); DArray<Edge>
           unintersected_edges; unintersected_edges.resize(unintersected_size);
