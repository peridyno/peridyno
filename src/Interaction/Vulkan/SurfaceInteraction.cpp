...
namespace dyno
{
    ...
    void SurfaceInteraction<TDataType>::calcSurfaceIntersectClick() {
        ...
        this->kernel("SurfaceRay")
            ->flush(vkDispatchSize(vk_num, 64), points.handle(), intersected.handle(), unintersected.handle(),
        ...
        this->kernel("Nearest")->flush(vkDispatchSize(vk_num, 64), intersected.handle(), unintersected.handle(),
                                       &vk_num, &vk_min_index);
        if (this->varToggleFlood()->getValue()) {
            ...
            while (intersected_size_t > intersected_size_t_o && intersected_size_t < triangles.size()) {
                ...
                this->kernel("NeighborTrisDiffuse")
                    ->flush(vkDispatchSize2D(vk_num, vk_num, 8), triangles.handle(), points.handle(),
        ...
        if (this->varToggleVisibleFilter()->getValue()) {
            vk_num.setValue(triangles.size());
            this->kernel("TriVisibleFilter")
                ->flush(vkDispatchSize(vk_num, 64), triangles.handle(), points.handle(), intersected.handle(),
        ...
        if (this->varToggleQuad()->getValue()) {
            vk_num.setValue(triangles.size());
            this->kernel("Tri2Quad")
                ->flush(vkDispatchSize(vk_num, 64), intersected.handle(), unintersected.handle(), &vk_num);
        }
        ...
        if (this->varToggleMultiSelect()->getData()) {
            if (this->triIntersectedIndex.size() == 0) {
                ...
                vkFill(*(this->triIntersectedIndex.handle()), 0);
            }
            ...
            this->kernel("Merge")->flush(vkDispatchSize(vk_num, 64), this->triIntersectedIndex.handle(),
        ...
        vk_num.setValue(triangles.size());
        this->kernel("SurfaceAssignOut")
            ->flush(vkDispatchSize(vk_num, 64), triangles.handle(), intersected_triangles.handle(),
        ...
        vkFill(*s2PSelected.handle(), 0);
        vk_num.setValue(outTriangleIndex.size());
        this->kernel("Surface2Point")
            ->flush(vkDispatchSize(vk_num, 64), triangles.handle(), outTriangleIndex.handle(), s2PSelected.handle(),
                    &vk_num);
        ...
        this->kernel("S2IndexOut")
            ->flush(vkDispatchSize(vk_num, 64), s2PSelected.handle(), s2PSelected_o.handle(), s2PSelectedIndex.handle(),
                    &vk_num);
        ...
        if (this->varToggleQuad()->getValue()) {
            ...
            vk_num.setValue(triangles.size());
            this->kernel("QuadOutput")
                ->flush(vkDispatchSize(vk_num, 64), intersected_o.handle(), intersected_q.handle(), &vk_num);
            ...
            this->kernel("QuadIndexOutput")
                ->flush(vkDispatchSize(vk_num, 64), outTriangleIndex.handle(), outQuadIndex.handle(), &vk_num);
    ...
    void SurfaceInteraction<TDataType>::calcSurfaceIntersectDrag() {
        ...
        this->kernel("SurfaceBox")
            ->flush(vkDispatchSize(vk_num, 64), points.handle(), triangles.handle(), intersected.handle(),
    ...
    void SurfaceInteraction<TDataType>::mergeIndex() {
        ...
        int intersected_size = thrust::reduce(thrust::device, intersected.begin(), intersected.begin() +
        intersected.size(), (int)0, thrust::plus<int>()); DArray<int> outTriangleIndex;
        outTriangleIndex.resize(intersected_size);
        thrust::exclusive_scan(thrust::device, intersected.begin(), intersected.begin() + intersected.size(),
        intersected.begin()); DArray<Triangle> intersected_triangles; intersected_triangles.resize(intersected_size);

        int unintersected_size = thrust::reduce(thrust::device, unintersected.begin(), unintersected.begin() +
        unintersected.size(), (int)0, thrust::plus<int>()); thrust::exclusive_scan(thrust::device,
        unintersected.begin(), unintersected.begin() + unintersected.size(), unintersected.begin()); DArray<Triangle>
        unintersected_triangles; unintersected_triangles.resize(unintersected_size);
        ...
        int s2PSelectedSize = thrust::reduce(thrust::device, s2PSelected.begin(), s2PSelected.begin() +
        s2PSelected.size(), (int)0, thrust::plus<int>()); DArray<int> s2PSelected_o; s2PSelected_o.assign(s2PSelected);

        thrust::exclusive_scan(thrust::device, s2PSelected.begin(), s2PSelected.begin() + s2PSelected.size(),
        s2PSelected.begin());
