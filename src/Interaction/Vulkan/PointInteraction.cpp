...
namespace dyno
{
    ...
    void PointInteraction<TDataType>::calcPointIntersectClick() {
        ...
        if (this->varToggleMultiSelect()->getData()) {
            ...
            this->kernel("Merge")
                ->flush(vkDispatchSize(vk_num, 64), this->pointIntersectedIndex.handle(), intersected.handle(),
        ...
        this->kernel("PointAssignOut")
            ->flush(vkDispatchSize(vk_num, 64), points.handle(), intersected_points.handle(),
    ...
    void PointInteraction<TDataType>::calcPointIntersectDrag() {
        ...
        this->kernel("PointBox")
            ->flush(vkDispatchSize(vk_num, 64), points.handle(), intersected.handle(), unintersected.handle(), &vk_num,
        ...
        if (this->varToggleMultiSelect()->getData()) {
            if (this->pointIntersectedIndex.size() == 0) {
                this->pointIntersectedIndex.resize(points.size());
                vkFill(*(this->pointIntersectedIndex.handle()), 0);
            }
            DArray<int> outIntersected;
            outIntersected.resize(intersected.size());
            DArray<int> outUnintersected;
    ...
    void PointInteraction<TDataType>::mergeIndex() {
        /*
           ...
           intersected.size(), (int)0, thrust::plus<int>()); DArray<int> outPointIndex;
            outPointIndex.resize(intersected_size);
            thrust::exclusive_scan(thrust::device, intersected.begin(), intersected.begin() + intersected.size(),
           intersected.begin()); DArray<Coord> intersected_points; intersected_points.resize(intersected_size);
           ...
           unintersected.begin(), unintersected.begin() + unintersected.size(), unintersected.begin()); DArray<Coord>
           unintersected_points; unintersected_points.resize(unintersected_size);
