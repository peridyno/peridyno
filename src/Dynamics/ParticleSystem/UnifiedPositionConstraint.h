...
namespace dyno {
	...
	/*!
	*	\class	UnifiedPositionConstraint
	*	\brief	This class implements a position-based solver for incompressibility.
	*/
	template<typename TDataType>
	class UnifiedPositionConstraint : public ConstraintModule
	{
		DECLARE_CLASS_1(UnifiedPositionConstraint, TDataType)
	public:
		typedef typename TDataType::Real Real;
		typedef typename TDataType::Coord Coord;

		UnifiedPositionConstraint();
		~UnifiedPositionConstraint() override;
	...
	template class UnifiedPositionConstraint<DataType3f>;
#else
 	template class UnifiedPositionConstraint<DataType3d>;
#endif
}