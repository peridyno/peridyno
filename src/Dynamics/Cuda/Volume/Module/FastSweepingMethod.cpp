#include "FastSweepingMethod.h"

namespace dyno
{
	IMPLEMENT_TCLASS(FastSweepingMethod, TDataType)
	...
	template<typename TDataType>
	void FastSweepingMethod<TDataType>::compute()
	{
	...
	template<typename TDataType>
	void FastSweepingMethod<TDataType>::loadClosedSurface()
	{
	...
	static void FSM_CheckNeighbour(const CArray<Vec3ui> &tri, const CArray<Vec3f> &vert,
		...
		int di, int dj, int dk)
	{
		...
		for (int k = k0; k != k1; k += dk) for (int j = j0; j != j1; j += dj) for (int i = i0; i != i1; i += di) {
			...
			FSM_CheckNeighbour(tri, x, phi, closest_tri, gx, i, j, k, i - di, j, k);
			FSM_CheckNeighbour(tri, x, phi, closest_tri, gx, i, j, k, i, j - dj, k);
			FSM_CheckNeighbour(tri, x, phi, closest_tri, gx, i, j, k, i - di, j - dj, k);
			FSM_CheckNeighbour(tri, x, phi, closest_tri, gx, i, j, k, i, j, k - dk);
			FSM_CheckNeighbour(tri, x, phi, closest_tri, gx, i, j, k, i - di, j, k - dk);
			FSM_CheckNeighbour(tri, x, phi, closest_tri, gx, i, j, k, i, j - dj, k - dk);
			FSM_CheckNeighbour(tri, x, phi, closest_tri, gx, i, j, k, i - di, j - dj, k - dk);
		}
	}
...
#define TRI_MAX(x, y, z) std::max(x, std::max(y, z))
	...
	template<typename TDataType>
	void FastSweepingMethod<TDataType>::makeLevelSet()
	{
