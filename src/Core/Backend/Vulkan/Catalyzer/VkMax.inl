...
namespace dyno {
	template<typename T>
	T VkMax<T>::reduce(const std::vector<T>& input)
	{
	...
	template<typename T>
	T VkMax<T>::reduce(const VkDeviceArray<T>& input)
	{
		...
		VkConstant<int> num;
		VkConstant<T> init_val {std::numeric_limits<T>::min()};
	...
	VkMax<T>::VkMax() {
		...
		mReduceKernel->load(getDynamicSpvFile<T>("shaders/glsl/core/Max.comp.spv"));
	}
}