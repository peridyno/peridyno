//
// Created by CreeperIclucia-Vader on 2025/10/27.
//
...
#ifndef UNIVERSAL_FUNC
#define UNIVERSAL_FUNC DYNO_HOST_DEVICE
#endif

#ifndef DYNO_HOST_ASM64
#define DYNO_HOST_ASM64 DYNO_ENABLE_HOST_ASM64
#endif

namespace dyno
{
    // ------------------------------------------------------------
    // Bit operations & small comparisons
    // ------------------------------------------------------------
    UNIVERSAL_FUNC inline int count_leading_zeros(uint64_t x)
    {
#ifdef __CUDA_ARCH__
        return __clzll(x);
    ...
    UNIVERSAL_FUNC inline int cmp_3_limbs(uint64_t a2, uint64_t a1, uint64_t a0,
    ...
    // ------------------------------------------------------------
    // Multiplication primitives (64-bit variants)
    // ------------------------------------------------------------
    UNIVERSAL_FUNC inline void mul64x64_to_128(uint64_t a, uint64_t b, uint64_t& lo, uint64_t& hi)
    {
    ...
    UNIVERSAL_FUNC inline void mul64x32_to_128(uint64_t a, uint32_t b, uint64_t& lo, uint64_t& hi)
    {
#if defined(__CUDA_ARCH__)
        asm volatile (
                "{\n\t"
        ...
        uint64_t p0 = a0 * b0;
        uint64_t p2 = a1 * b0;
        uint64_t middle = (p0 >> 32) + (p2 & 0xFFFFFFFFULL);
        hi = (p2 >> 32) + (middle >> 32);
        lo = (middle << 32) | (p0 & 0xFFFFFFFFULL);
#endif
    }

    UNIVERSAL_FUNC inline void umul64(uint64_t a, uint64_t b,
                           uint64_t& hi, uint64_t& lo)
    {
        mul64x64_to_128(a, b, lo, hi);
    }

    UNIVERSAL_FUNC inline uint64_t umullo64(uint64_t a, uint64_t b)
    {
#if defined(__CUDA_ARCH__)
        uint64_t lo;
        asm volatile (
    ...
    UNIVERSAL_FUNC inline uint64_t umulhi64(uint64_t a, uint64_t b)
    {
        uint64_t hi;
#if defined(__CUDA_ARCH__)
        asm volatile (
    ...
    // ------------------------------------------------------------
    // 128-bit add/sub primitives
    // ------------------------------------------------------------
    UNIVERSAL_FUNC inline void add128(uint64_t ahi, uint64_t alo,
    ...
    // add128 but also outputs carry (1 if carry out)
    UNIVERSAL_FUNC inline void add128_with_carry(uint64_t ahi, uint64_t alo,
                                  uint64_t bhi, uint64_t blo,
                                  uint64_t& rhi, uint64_t& rlo,
                                  uint64_t& carry_out)
    {
        ...
        asm volatile (
                ...
                "add.cc.u64 %0, %3, %4;\n\t"   // rlo
                "addc.u64   %1, %5, %6;\n\t"   // rhi
                "addc.u64   %2, 0, 0;\n\t"     // carry = 1 if carry out, else 0
                "}\n\t"
                : "=l"(rlo), "=l"(rhi), "=l"(carry)
                : "l"(alo), "l"(blo), "l"(ahi), "l"(bhi)
            );
        carry_out = carry;
#elif DYNO_HOST_ASM64
        uint64_t tlo = alo, thi = ahi;
        ...
        rlo = tlo;
        rhi = thi;
        carry_out = (uint64_t)cflag;
#else
        rlo = alo + blo;
        uint64_t c1 = (rlo < alo);
        rhi = ahi + bhi + c1;
        uint64_t c2 = (rhi < ahi) || (c1 && rhi == ahi);
        carry_out = c2;
#endif
    }

    UNIVERSAL_FUNC inline void sub128(uint64_t ahi, uint64_t alo,
                       uint64_t bhi, uint64_t blo,
                       uint64_t& rhi, uint64_t& rlo)
    {
#if defined(__CUDA_ARCH__)
    ...
    // Like sub128 but also returns borrow (1 if borrow occurred)
    UNIVERSAL_FUNC inline void sub128_with_borrow(uint64_t ahi, uint64_t alo,
                                  uint64_t bhi, uint64_t blo,
                                  uint64_t& rhi, uint64_t& rlo,
                                  uint64_t& borrow_out)
    {
        ...
        asm volatile (
                ...
                "sub.cc.u64 %0, %3, %4;\n\t"   // rlo
                "subc.u64   %1, %5, %6;\n\t"   // rhi
                "addc.u64   %2, 0, 0;\n\t"     // carry = 1 if no borrow, 0 if borrow
                "}\n\t"
                : "=l"(rlo), "=l"(rhi), "=l"(carry)
                : "l"(alo), "l"(blo), "l"(ahi), "l"(bhi)
            );
        borrow_out = carry ? 0ULL : 1ULL;
#elif DYNO_HOST_ASM64
        uint64_t tlo = alo, thi = ahi;
        unsigned char cflag;
        asm volatile (
        ...
        rlo = tlo;
        rhi = thi;
        borrow_out = (uint64_t)cflag;
#else
        rlo = alo - blo;
        uint64_t b1 = (alo < blo);
        rhi = ahi - bhi - b1;
        borrow_out = (ahi < bhi) || (b1 && ahi == bhi);
#endif
    }

    // ------------------------------------------------------------
    // Masks & comparisons across limbs
    // ------------------------------------------------------------
    UNIVERSAL_FUNC inline uint64_t mask_from_bool(bool b)
    {
        return (uint64_t)0 - (uint64_t)(b ? 1 : 0);
    }

    UNIVERSAL_FUNC inline uint64_t abs_greater_mask(uint64_t hi1, uint64_t lo1,
                                                    uint64_t hi2, uint64_t lo2)
    {
    ...
    UNIVERSAL_FUNC inline uint64_t abs_greater_mask(uint64_t a2, uint64_t a1, uint64_t a0,
                                                    uint64_t b2, uint64_t b1, uint64_t b0)
    {
    ...
    UNIVERSAL_FUNC inline uint64_t abs_ge_mask_u(uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3,
    ...
    UNIVERSAL_FUNC inline bool abs_gt_u(uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3,
                                        uint64_t b0, uint64_t b1, uint64_t b2, uint64_t b3)
    {
    ...
    UNIVERSAL_FUNC inline bool lt256_u(uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3,
                                       uint64_t b0, uint64_t b1, uint64_t b2, uint64_t b3)
    {
        if (a3 != b3) return a3 < b3;
        if (a2 != b2) return a2 < b2;
    ...
    UNIVERSAL_FUNC inline bool eq256_u(uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3,
                                       uint64_t b0, uint64_t b1, uint64_t b2, uint64_t b3)
    {
        return (a0 == b0) && (a1 == b1) && (a2 == b2) && (a3 == b3);
    }

    UNIVERSAL_FUNC inline bool greater_than_4_limbs(uint64_t a3, uint64_t a2, uint64_t a1, uint64_t a0,
    ...
    UNIVERSAL_FUNC inline bool cmp_5_limbs(uint64_t a4, uint64_t a3, uint64_t a2, uint64_t a1, uint64_t a0,
    ...
    UNIVERSAL_FUNC inline bool addition_will_overflow(uint64_t a, uint64_t b)
    {
        return a > UINT64_MAX - b;
    }
    UNIVERSAL_FUNC inline bool addition_will_overflow(uint64_t a, uint64_t b, uint64_t c)
    {
    ...
    // ------------------------------------------------------------
    // Division helpers & reciprocal precomputation
    // ------------------------------------------------------------
    // Algorithm 2 : RECIPROCAL_WORD_64
    UNIVERSAL_FUNC inline uint64_t RECIPROCAL_WORD_64(uint64_t d)
    {
    ...
    UNIVERSAL_FUNC inline void div128_by_64(uint64_t u1, uint64_t u0,
    ...
    UNIVERSAL_FUNC inline Div64Reciprocal precompute_div64_reciprocal(uint64_t d)
    {
        assert(d != 0);
        unsigned lz = count_leading_zeros(d);
        uint64_t d_norm = d << lz;
        uint64_t v = RECIPROCAL_WORD_64(d_norm);
        return {d_norm, v, static_cast<uint32_t>(lz)};
    }

    UNIVERSAL_FUNC inline void div128by64_precomputed(uint64_t hi, uint64_t lo,
                                                      const Div64Reciprocal& rc,
                                                      uint64_t& q, uint64_t& r)
    {
        uint64_t u1_norm = rc.shift ? ((hi << rc.shift) | (lo >> (64 - rc.shift))) : hi;
        uint64_t u0_norm = lo << rc.shift;
        div128_by_64(u1_norm, u0_norm, rc.d_norm, rc.reciprocal, q, r);
        r >>= rc.shift;
    }
    ...
    {
        uint64_t r;
        div128by64_precomputed(hi, lo, rc, q, r);
    }

    UNIVERSAL_FUNC inline void div128_by_64_direct(uint64_t u1, uint64_t u0,
                                    uint64_t d,
                                    uint64_t& q, uint64_t& r)
    {
    ...
    UNIVERSAL_FUNC inline void div128by64_rem(uint64_t hi, uint64_t lo,
                               uint64_t d,
                               uint64_t& q, uint64_t& r)
    {
        assert(d != 0);
    ...
    UNIVERSAL_FUNC inline void div128by64(uint64_t hi, uint64_t lo,
                           uint64_t d,
                           uint64_t& q)
    {
        assert(d != 0);
    ...
    UNIVERSAL_FUNC inline void div128by64(uint64_t hi, uint64_t lo, uint64_t d, uint64_t& ret_hi, uint64_t& ret_lo)
    {
        ret_hi = hi / d;
        hi %= d;
        uint64_t r;
    ...
    UNIVERSAL_FUNC inline void div128by64_rem(uint64_t hi, uint64_t lo, uint64_t d, uint64_t& ret_hi, uint64_t& ret_lo, uint64_t& r)
    {
        ret_hi = hi / d;
        hi %= d;
        // Need remainder adjusted for normalization; reuse rem version
        div128by64_rem(hi, lo, d, ret_lo, r);
    }

    // inline void div192by128_rem(...) // kept commented for reference
} // namespace dyno

// CUDA-specific vectorized functions - only available when compiling with CUDA
#if defined(__CUDA_ARCH__) && DYNO_ENABLE_PTX
...
#endif // defined(__CUDA_ARCH__) && DYNO_ENABLE_PTX
