...
namespace dyno
{
	...
	MedialSlabModel<TDataType>::MedialSlabModel()
		: BasicShape<TDataType>()
	{
		...
		auto callback = std::make_shared<FCallBackFunc>(std::bind(&MedialSlabModel<TDataType>::varChanged, this));
	...
	template<typename TDataType>
	void MedialSlabModel<TDataType>::resetStates()
	{
		varChanged();
	}

	template<typename TDataType>
	NBoundingBox MedialSlabModel<TDataType>::boundingBox()
	{
	...
	template<typename TDataType>
	void MedialSlabModel<TDataType>::varChanged()
	{
		...
		TMedialSlab3D<Real> medialSlabPrim = TMedialSlab3D<Real>(pA_world, pB_world, pC_world, rA, rB, rC);
	...
	template<typename TDataType>
	double MedialSlabModel<TDataType>::compute_angle(double r1, double r2, const Vec3f& c21) {
	...
	template<typename TDataType>
	Vec3f MedialSlabModel<TDataType>::plane_line_intersection(const Vec3f& n, const Vec3f& p,
	...
	template<typename TDataType>
	Mat4f MedialSlabModel<TDataType>::rotate_mat(const Vec3f& point, const Vec3f& axis, double angle) {
	...
	template<typename TDataType>
	Vec3f MedialSlabModel<TDataType>::mat_vec_mult(const Mat4f& mat,
		const Vec3f& vec)
	{
	...
	template<typename TDataType>
	std::vector<Vec3f> MedialSlabModel<TDataType>::intersect_point_of_cones(const Vec3f& v1, double r1,
	...
	template<typename TDataType>
	int MedialSlabModel<TDataType>::generate_slab(
		const Vec3f& v1,
		double r1,
		const Vec3f& v2,
		double r2,
	...
	template<typename TDataType>
	void MedialSlabModel<TDataType>::generate_conical_surface(
		const Vec3f& v1,
		float r1,
		const Vec3f& v2,
		float r2,
