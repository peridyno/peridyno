...
WtNode& WtFlowScene::createNode(std::unique_ptr<WtNodeDataModel>&& dataModel, Wt::WPainter* painter, int selectType)
{
	Wt::WPaintDevice* paintDevice = painter->device();

	auto node = detail::make_unique<WtNode>(std::move(dataModel), paintDevice);
	auto ngo = detail::make_unique<WtNodeGraphicsObject>(*this, *node, painter, selectType);
