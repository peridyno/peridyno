...
using QtNodes::QtFlowView;
using QtNodes::QtFlowScene;

QtFlowView::
QtFlowView(QWidget *parent)
...
QtFlowView::
QtFlowView(QtFlowScene *scene, QWidget *parent)
  : QtFlowView(parent)
{
  setScene(scene);
}
...
void
QtFlowView::setScene(QtFlowScene *scene)
{
  _scene = scene;
  ...
  connect(_deleteSelectionAction, &QAction::triggered, this, &QtFlowView::deleteSelectedNodes);
...
deleteSelectedNodes()
{
  ...
  {
    if (auto c = qgraphicsitem_cast<QtConnectionGraphicsObject*>(item))
  ...
  for (QGraphicsItem * item : _scene->selectedItems())
  {
    if (auto n = qgraphicsitem_cast<QtBlockGraphicsObject*>(item))
