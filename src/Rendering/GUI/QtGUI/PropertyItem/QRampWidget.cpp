...
namespace dyno
{
		...
		: QFieldWidget(field)
	{
		...
		{	
			if(f->getValue().useColseButton && DrawLabel->borderMode == QDrawLabel::BorderMode::Open)
			{
				...
				connect(curveCloseButton, &QToggleButton::clicked, DrawLabel, &QDrawLabel::setCurveClose);
				curveCloseButton->setValue(f->getValue().curveClose);
			}
		}


		QLabel* NameUseRamp = new QLabel();
...

		{
			if (f->getValue().displayUseRamp == true)
			{
				...
				{
					NameUseRamp->setText("UseCurve");
					printf("open\n");
				}
				else if (f->getValue().Bordermode == Ramp::BorderMode::Close) 
				{
					NameUseRamp->setText("UseRamp");
					printf("close\n");
				}

				CheckboxUseRamp->nameLabel = NameUseRamp;
				CheckboxUseRamp->QWidget::setFixedWidth(20);
				CheckboxUseRamp->QAbstractButton::setChecked(f->getValue().useRamp);
		...
		HlayoutUseRamp->addWidget(NameUseRamp);
		HlayoutUseRamp->addWidget(CheckboxUseRamp);
	...
	void QDrawLabel::paintEvent(QPaintEvent* event)
	{
		...
		if (CoordArray.empty()) 
		{
			if (field->getValue().Originalcoord.empty())		//如果field中没有Widget传回的数据，则从field本身的Coord进行初始化
			{
				...
				else if (this->borderMode == BorderMode::Open)
				{
		...
		if (useBezier)		//画贝塞尔曲线或折线
		{
			...
			if (curveClose && reSortCoordArray.size()>=3 && this->borderMode == BorderMode::Open)
			{
	...
	void QDrawLabel::mousePressEvent(QMouseEvent* event) 
	{
		...
		{
			//close模式下插入点并自动排序
			if ((borderMode == BorderMode::Close) )
			{
				addPointtoEnd();
			}
			//尾插
			else if (borderMode == BorderMode::Open && !ctrlKey)
			{
				addPointtoEnd();
			}
			//线段中间插入点
		...
		{
			if (this->borderMode == BorderMode::Close && selectPoint > 1) 
			{
				deletePoint();
			}
			else if(this->borderMode == BorderMode::Open &&selectPoint >=0)
			{
				deletePoint();
			}
	...
	int QDrawLabel::addPointtoEnd() 
	{
		...
		else if (borderMode == BorderMode::Open) 
		{
			if (!InsertAtBegin)
			{
				CoordArray.push_back(pressCoord);
		...
		if (isSelect) 
		{
			...
			if (multiSelectID.size() > 1)
			{
				for (size_t i = 0; i < multiSelectID.size(); i++)
				{
					...
					{
						if (borderMode == BorderMode::Close && tempSelect <= 1)
						{
							if (Mode == Dir::x)
							{
	...
	void QDrawLabel::CoordtoField(Ramp& s)
	{
		...
		if (this->borderMode == BorderMode::Open) 
		{
		...
		if (borderMode == BorderMode::Close) { s.Bordermode = Ramp::BorderMode::Close; }
		else { s.Bordermode = Ramp::BorderMode::Open; }
	...
	void QDrawLabel::copySettingFromField()
	{
		...
		if (field->getValue().Bordermode == Ramp::BorderMode::Close) { borderMode =Close; }
		else { borderMode = Open; }

		if (field->getValue().Dirmode == Ramp::Direction::x) { Mode = Dir::x; }
		else { Mode = Dir::y; }
		if (field->getValue().Bordermode == Ramp::BorderMode::Close){ useSort = true; }
		else { useSort = false; }
