...
namespace dyno
{
	...
	void TextureMeshLoader::callbackLoadFile()
	{
		...
		mInitialVertex.assign(texMesh->meshDataPtr()->vertices());
		mInitialNormal.assign(texMesh->meshDataPtr()->normals());
		mInitialTexCoord.assign(texMesh->meshDataPtr()->texCoords());
