#ifndef RAMP_SERIALIZATION
#define RAMP_SERIALIZATION
...
namespace dyno {
	...
	template<>
	inline std::string FVar<Ramp>::serialize()
	{

		std::string finalText;
		//serialize Array
		this->getValue().convertCoordToStr("UserPoints", this->getValue().Ramp::getUserPoints(), finalText);
		this->getValue().convertCoordToStr("UserHandles", this->getValue().Ramp::getUserHandles(), finalText);
	...
	template<>
	inline bool FVar<Ramp>::deserialize(const std::string& str)
	{
		if (str.empty())
			return false;
		...
		while (ss >> substr)
		{
			...
			if (currentVarName == "UserPoints")
			{
				...
				{
					ramp->getUserPoints().push_back(Ramp::Coord2D(tempCoord, std::stod(substr)));
				}
			}
			else if (currentVarName == "UserHandles")
			{
				...
				{
					ramp->getUserHandles().push_back(Ramp::Coord2D(tempCoord, std::stod(substr)));
				}
			}
