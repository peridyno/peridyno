...
private:									\
	FVar<T> var_##name = FVar<T>(T(value), std::string("var_") + std::string(#name), desc, FieldTypeEnum::Param, this);			\
public:										\
	inline FVar<T>* var##name() {return &var_##name;}

#define DEF_VAR_IN(T, name, desc) \
private:									\
	FVar<T> in_##name = FVar<T>(std::string("in_") + std::string(#name), desc, FieldTypeEnum::In, this);			\
public:										\
	inline FVar<T>* in##name() {return &in_##name;}

#define DEF_VAR_OUT(T, name, desc) \
private:									\
	FVar<T> out_##name = FVar<T>(std::string("out_") + std::string(#name), desc, FieldTypeEnum::Out, this);			\
public:									\
...
private:									\
	FInstance<T> in_##name = FInstance<T>(std::string("in_") + std::string(#name), desc, FieldTypeEnum::In, this);			\
public:										\
	inline FInstance<T>* in##name() {return &in_##name;}

#define DEF_INSTANCE_OUT(T, name, desc) \
private:									\
	FInstance<T> out_##name = FInstance<T>(std::string("out_") + std::string(#name), desc, FieldTypeEnum::Out, this);			\
public:									\
...
private:									\
	FArray<T, device> out_##name = FArray<T, device>(std::string("out_") + std::string(#name), desc, FieldTypeEnum::Out, this);	\
public:									\
	inline FArray<T, device>* out##name() {return &out_##name;}

#define DEF_ARRAY_IO(T, name, device, desc) \
private:									\
	FArray<T, device> io_##name = FArray<T, device>(std::string("io_") + std::string(#name), desc, FieldTypeEnum::IO, this);	\
public:									\
...
private:												\
	FArray2D<T, device> in_##name = FArray2D<T, device>(std::string("in_") + std::string(#name), desc, FieldTypeEnum::In, this);	\
public:													\
	inline FArray2D<T, device>* in##name() {return &in_##name;}

#define DEF_ARRAY2D_OUT(T, name, device, desc)		\
private:												\
	FArray2D<T, device> out_##name = FArray2D<T, device>(std::string("out_") + std::string(#name), desc, FieldTypeEnum::Out, this);	\
public:													\
	inline FArray2D<T, device>* out##name() {return &out_##name;}

#define DEF_ARRAY2D_IO(T, name, device, desc)		\
private:												\
	FArray2D<T, device> io_##name = FArray2D<T, device>(std::string("io_") + std::string(#name), desc, FieldTypeEnum::IO, this);	\
public:													\
...
private:												\
	FArray3D<T, device> in_##name = FArray3D<T, device>(std::string("in_") + std::string(#name), desc, FieldTypeEnum::In, this);	\
public:													\
	inline FArray3D<T, device>* in##name() {return &in_##name;}

#define DEF_ARRAY3D_OUT(T, name, device, desc)		\
private:												\
	FArray3D<T, device> out_##name = FArray3D<T, device>(std::string("out_") + std::string(#name), desc, FieldTypeEnum::Out, this);	\
public:													\
	inline FArray3D<T, device>* out##name() {return &out_##name;}

#define DEF_ARRAY3D_IO(T, name, device, desc)		\
private:												\
	FArray3D<T, device> io_##name = FArray3D<T, device>(std::string("io_") + std::string(#name), desc, FieldTypeEnum::IO, this);	\
public:													\
...
private:												\
	FArrayList<T, device> in_##name = FArrayList<T, device>(std::string("in_") + std::string(#name), desc, FieldTypeEnum::In, this);	\
public:													\
	inline FArrayList<T, device>* in##name() {return &in_##name;}

#define DEF_ARRAYLIST_OUT(T, name, device, desc)		\
private:												\
	FArrayList<T, device> out_##name = FArrayList<T, device>(std::string("out_") + std::string(#name), desc, FieldTypeEnum::Out, this);	\
public:													\
	inline FArrayList<T, device>* out##name() {return &out_##name;}

#define DEF_ARRAYLIST_IO(T, name, device, desc)		\
private:												\
	FArrayList<T, device> io_##name = FArrayList<T, device>(std::string("io_") + std::string(#name), desc, FieldTypeEnum::IO, this);	\
public:													\
