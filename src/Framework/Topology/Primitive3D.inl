...
namespace dyno
{
	...
	template<typename Real>
	DYN_FUNC Real TPoint3D<Real>::areaTriangle(const TTriangle3D<Real>& triangle, const Real& r) const
	{
		Coord3D q = project(triangle).origin;
		...
		for (int j = 0; j < 3; j++)
		{
			...
			if ((distance(so)) < r - EPSILON)
			{
				...
				Real coco = (plo1 - p2.origin).dot(plo2 - p2.origin) / (plo2 - p2.origin).norm() / (plo1 - p2.origin).norm();
				if (coco > 1.0f) coco = Real(1) - EPSILON;
				if (coco < -1.0f) coco = -Real(1) + EPSILON;
	...
	template<typename Real>
	DYN_FUNC Real TPoint3D<Real>::areaTrianglePrint(const TTriangle3D<Real>& triangle, const Real& r) const
	{
		Coord3D q = project(triangle).origin;
