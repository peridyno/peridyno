...
layout (location = 5) in float halfLength;
...
void main () 
{
	...
	vec3 localPos = inPos * vec3(radius, radius, radius);
	if (localPos.x > 0) {
		localPos.x += halfLength;
	} else {
		localPos.x -= halfLength;
	}
	vec3 iPos = quat_rotate(inTransRotation, localPos) + inTransPosition;
